...
class BOQVisibilityServiceTests(TestCase):
    ...
    def test_superadmin_sees_only_own(self):
        ids = set(boqs_visible_to_user(self.superadmin).values_list("pk", flat=True))
        self.assertEqual(ids, {self.boq_super.pk})

    def test_admin_sees_own_and_all_experts(self):
        ids = set(boqs_visible_to_user(self.admin).values_list("pk", flat=True))
        self.assertEqual(
            ids,
            {self.boq_admin.pk, self.boq_expert.pk, self.boq_expert2.pk},
        )
        self.assertNotIn(self.boq_admin2.pk, ids)
        self.assertNotIn(self.boq_super.pk, ids)

    def test_admin_can_open_expert_boq_detail(self):
    ...
    def test_superadmin_cannot_open_expert_boq(self):
