...
Per-unit material cost is the selected Rate_Master row's purchase rate
(docs/PRD.md - Internal Review Sheet: Purchase rate; Cost Calculation). The
matched row is stored on the ProductMatch. Deterministic - AI never performs
costing (docs/AGENTS.md - AI Rules).
"""
...
def material_cost(product_match) -> Decimal:
    ...
    Zero when no product/rate is selected (e.g. pending products), so the row
